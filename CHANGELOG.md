...
- Added PlaceholderAPI values for KDR, win rate, queued players, and FFA players.
...
- Added a configurable main hub menu with quick access to ranked, unranked, party, profile, and stats flows.
- Added direct player commands for common menus: `/ffamenu`, `/ffastats`, `/ffatop`, `/ffaranks`, `/ranked`, and `/unranked`.
- Added `/ffa` subcommands for the supported menu flows, including `menu`, `ffa`, `arenas`, `ranks`, and `party`.
- Centered ranked and unranked queue menu items by default, with configurable item slots.
- Added more default kill effects.
- Improved menu protection for filler clicks and double-click item collection.

## [1.3.2 - 1.3.5]

- Added a roadmap/TODO doc for future ideas like a custom kit editor.
...
- Use [docs/FEATURES.md](docs/FEATURES.md) for feature scope.
- Use [docs/PROXY.md](docs/PROXY.md) for proxy installs.
